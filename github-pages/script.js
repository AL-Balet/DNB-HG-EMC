...
async function loadExercises() {
  if (Array.isArray(window.DNB_EXERCISES) && window.DNB_EXERCISES.length) {
    exercises.splice(0, exercises.length, ...window.DNB_EXERCISES);
    return;
  }

  const response = await fetch(`./data.json?v=1`, { cache: "no-store" });
  if (!response.ok) {
...
async function initApp() {
  ...
  try {
    ...
    const localHint = window.location.protocol === "file:"
      ? " Pour un test local, ouvrez le site avec un petit serveur web ; sur GitHub Pages, le chargement fonctionnera normalement."
      : "";
    setLoadingState(`Impossible de charger data.json.${localHint}`);
  }
}
